    ...
    <div class="rounded-lg bg-card text-card-foreground">
        <div class="flex flex-col space-y-1.5">
            <div class="flex items-center justify-between gap-x-2 text-base">
                <h3 class="inline-flex items-center justify-center gap-x-1 font-semibold leading-none">${renderOptionalLink(work.company, work.link, "hover:underline")}<span
                ...
                <div class="text-sm tabular-nums text-gray-500">${escapeHTML(work.start)} - ${escapeHTML(work.end)}</div>
            </div>
            <h4 class="font-mono text-sm leading-none">${escapeHTML(work.title)}</h4>
        </div>
        <div class="text-pretty font-mono text-muted-foreground mt-2 text-xs">${escapeHTML(work.description)}</div>
    </div>
    `;
}).join("");
